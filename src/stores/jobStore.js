...
const useJobStore = create(
  devtools(
    persist((set, get) => ({
      ...
      optimisticUpdates: new Map(), // track pending updates for rollback
        ...
        try {
          ...
          set({
            ...
            pagination: {
              ...
              totalPages:
                meta.totalPages ??
                Math.max(1, Math.ceil((meta.total ?? list.length) / pageSize)),
            },
            loading: false,
          });
      ...
      toggleJobStatus: async (jobId, currentStatus) => {
        ...
        optimisticUpdates.set(jobId, { ...originalJob });
        ...
        set({ jobs: optimisticJobs, optimisticUpdates });
        get().filterJobs();
        ...
        try {
          ...
          optimisticUpdates.delete(jobId);
          set({ jobs: serverJobs, optimisticUpdates });
          get().filterJobs();

          return updatedJob;
        } catch (error) {
          // Rollback on failure
          const rollbackJob = optimisticUpdates.get(jobId);
      ...
      reorderJobs: async (activeId, overId) => {
        ...
        // Mark as pending for optimistic update
        optimisticUpdates.set(
          activeId,
          oldJobs.find((j) => j.id === activeId)
        );
        ...
        try {
          ...
          optimisticUpdates.delete(activeId);
